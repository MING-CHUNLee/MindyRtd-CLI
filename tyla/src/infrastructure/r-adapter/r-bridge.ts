 ...
 * Service: R Bridge
 *
 * File-based communication with the Tyla R package running in RStudio.
...
function getPendingFile(): string {
    return path.join(getTylaDir(), 'pending.json');
}

function getResultFile(): string {
    return path.join(getTylaDir(), 'result.json');
}

function getLockFile(): string {
    return path.join(getTylaDir(), '.lock');
}
